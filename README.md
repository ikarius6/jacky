...
- **AI Collectibles** — discover unique items randomly dropped on your screen, with dynamic lore and rarity generated by the LLM
- **Conversation Memory** — Jacky remembers recent exchanges so follow-up questions feel natural and coherent; configurable window size and optional disk persistence
...
When LLM is enabled, Jacky keeps a rolling history of recent user ↔ Jacky exchanges and injects them into every chat request as proper multi-turn messages. This lets you have natural follow-up conversations:

```
You:   "give me a difficult math problem"
Jacky: "ok, try 273 + 127"
You:   "it's 400!"
Jacky: "that's correct! 🎉"
```

Memory is **scoped to direct questions** only — idle comments, auto-reactions (battery, music, collectibles), and screen-interaction prompts are intentionally excluded so they don't pollute the chat context.

| Config key | Default | Description |
|---|---|---|
| `memory_max_turns` | `5` | Number of user ↔ assistant exchange pairs to keep (1–50) |
| `memory_persist` | `false` | If `true`, history is saved to disk and survives restarts |

Both settings are also available in **Settings → LLM → Conversation Memory** without editing any JSON.
...
While running in the background, Jacky will occasionally spawn random collectible items on your screen. If you click on one:
- The item is added to your permanent Collection.
- Jacky uses the LLM to generate a completely unique, humorous description ("lore") and a rarity rating (1 to 5 stars) for that specific instance.
- This means even if you find the same object twice (e.g., a Rubber Duck), each one will have a unique backstory and rarity!
- 30+ different sprites to discover, collect them all!

You can view all your discovered items in the Collection Panel via the context menu.
